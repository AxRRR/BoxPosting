...
const Noticias = () => {
  ...
  const ListaGeneroInternacional = async () => {
    const urlinter = await fetch('http://localhost:8080/api/buscarnoticiasporgenero/2')
    const noticiainter = await urlinter.json()
    setInternacional(noticiainter)
  }

  const ListaGeneroCineySeries = async () => {
    const urlinter = await fetch('http://localhost:8080/api/buscarnoticiasporgenero/3')
    const noticiainter = await urlinter.json()
    setCineySeries(noticiainter)
  }

  const ListaGeneroVideojuegos = async () => {
    const urlinter = await fetch('http://localhost:8080/api/buscarnoticiasporgenero/5')
    const noticiainter = await urlinter.json()
    setVideojuegos(noticiainter)
  }

  const ListaGeneroDeportes = async () => {
    const urlinter = await fetch('http://localhost:8080/api/buscarnoticiasporgenero/6')
    const noticiainter = await urlinter.json()
    setDeportes(noticiainter)
  }

  const ListaGeneroAnime = async () => {
    const urlinter = await fetch('http://localhost:8080/api/buscarnoticiasporgenero/7')
    const noticiainter = await urlinter.json()
        ...
        <h2 className={classes.Noticias_Titulo}>Últimas noticias:</h2>
            <ul className={classes.Noticias_BOXStyle}>
                ...
                className={classes.Noticias_Caja}> 
                    <Link to={"/ver/noticia/" + news.id}><img src={news.imagen_noticia}
                    alt={news.imagen_noticia} className={classes.Noticias_BOXImg} /></Link>
            ...
            <h2 className={classes.Noticias_Titulo}>Últimas noticias tecnologia:</h2>
            <ul className={classes.Noticias_BOXStyle}>
                <div>
            ...
            <h2 className={classes.Noticias_Titulo}>Últimas noticias internacional:</h2>
            <ul className={classes.Noticias_BOXStyle}>
                <div>
            ...
            <h2 className={classes.Noticias_Titulo}>Últimas noticias cine y series:</h2>
            <ul className={classes.Noticias_BOXStyle}>
                <div>
            ...
            <h2 className={classes.Noticias_Titulo}>Últimas noticias videojuegos:</h2>
            <ul className={classes.Noticias_BOXStyle}>
                <div>
